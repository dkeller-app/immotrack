...
| 32 | ARCHI-DB-DOUBLONS | Refonte architecture DB — séparer log (bien physique) et bail (contrat), bidirectionnel | P1 | XL (~12-15h) | 🔄 **Phase 1 CDC ✅ livrée v14.13** · Phases 2-4 prêtes à coder | [docs/subjects/ARCHI-DB-DOUBLONS.md](docs/subjects/ARCHI-DB-DOUBLONS.md) · **Phase 1 CDC livrée 2026-05-02 commit `ae73859`** : 10 décisions arbitrées (Q1-Q8 + Q2bis + Q4bis), audit code complet (165 sites + 5 fonctions PDF), 12 champs à créer sur log dont `log.typeUsage` (NEW) + `log.mobilier` (NEW), 7 helpers à créer Phase 2, 33 champs à retirer Phase 4. **Phase 2 = ~3-4h** migration data + getters. **Phase 3 = ~6-8h** refacto reads/writes (splittable 3a UI / 3b PDF). **Phase 4 = ~1-2h** cleanup. Prompt Phase 2 sur-mesure prêt à utiliser. |
