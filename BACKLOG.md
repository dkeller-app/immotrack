...
| **Relay-401 câblage** — smoke user 11/08 : la boucle 401 PERSISTE en v15.500 (13 liens échus juin-juillet re-tirés en boucle). Cause : le fix v15.498 (`3e66359`) a construit+testé+exposé `partitionCandidatLinks` **sans jamais la brancher** dans `_relayPullCandidatures` (fix inerte en prod). | ✅ **FIX CONSTRUIT + AUDITÉ « SÛR À TESTER » 11/08** : worktree `Immo-wt-relay401` (`fix/relay401-cablage`, `6c830d3`, **v15.501**) — partition branchée (échus+7j clos SANS réseau + toast pull manuel) + ceinture 401-sur-lien-échu = clos (401 sur lien vivant = erreur retentée, token désync ≠ mort) + repli défensif + 1 test (2250/2250). Vérifié navigateur stub réseau + vrais boutons : échus clos sans requête, re-run 0 requête, grâce/vivants tirés, données user = 13 échus datés (0 legacy). Bonus : répare la persistance des clôtures 404 (saveDB manquant). Audit : 0 critique/important, 2 mineurs consignés (auto-pull réveille encore le timer à vide — follow-on propre ; toast partiel si import+clôture simultanés) | **🚀 GO user 11/08 → PUSHÉ + DÉPLOYÉ v15.501** (`d331162`, version live vérifiée github.io). Reste : smoke user (recharger l'app → premier pull clôt les 13 liens échus → console propre) → destruction worktree `Immo-wt-relay401` |
