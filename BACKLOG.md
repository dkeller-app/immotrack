...
| 32 | ARCHI-DB-DOUBLONS | Refonte architecture DB — séparer log (bien physique) et bail (contrat), bidirectionnel | P1 | XL (~12-15h, ~3-5h restantes) | 🔄 **Phases 1+2+3a+3b+4a ✅ livrées v14.17** · Phase 4b ⬜ cleanup final | [docs/subjects/ARCHI-DB-DOUBLONS.md](docs/subjects/ARCHI-DB-DOUBLONS.md) · **Phase 1 CDC** `ae73859`. **Phase 2** `511faf3` v14.14. **Phase 3a UI tabs** `5d7097f` v14.15. **Phase 3b sync étendu** `17426cf` v14.16 (pivot : `_syncLogToBail` propage 24 champs vers bail courant non signé). **Phase 4a wizard bail readonly** `5fd2ca0` v14.17 (`_makeBailBienTabReadOnly` applique readonly/disabled aux ~60 inputs du tab bp-bien, CSS dashed/opacity, source unique = modale logement Phase 3a). **Phase 4b = ~3-5h** vraie refacto reads/writes (149 sites → `_readLogForBail` + getCurrentBailFor/Tenant/Rent dans rBaux, rMv, rDash, rEDLList, rQuit, fiches 360°) + 5 fonctions PDF + suppression définitive 33 champs `bail.X` obsolètes + suppression `_syncLogToBail`. **Session future avec budget tests dédié** (risque PDF élevé). |
