...
### FEAT-GEORISQUES-ERP 🗺️ — Détection auto ERP/IAL depuis l'adresse (2026-06-02, v15.252) — P2
...
> - **Vérif** : check-inline-js 4/0 · `node tools/sync-helpers-global-mirrors.mjs` OK · **1385/1385 Vitest**. **🛡 Audit `superpowers:code-reviewer` ×2 → SHIP** : (1) initial 0 BLOCKER (légal, XSS escapé, non-clobber, anti-boucle re-render, **immutabilité bail signé confirmée**) + 3 NITs ; (2) re-audit du fix `pprt/pprm` (5 signaux, comble un faux « non requis » pour commune Seveso/minière) → SHIP régression-free. **⏳ EN ATTENTE TEST VISUEL** (sandbox). PROD `index.html`+`sw.js` PAS touchés (sync différée avec Phase B).
