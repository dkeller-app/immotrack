...
>
> **[BUG-PARTAGE-EDL-ESPACE-TIERS](docs/subjects/BUG-PARTAGE-EDL-ESPACE-TIERS.md) (P1 / S-M)** — **✅ FIX PRÊT À INTÉGRER 10/08 + EDL RESTAURÉ** (audits code-reviewer ×2 → **SÛR** ; branche `origin/fix/partage-edl-espace-tiers` `083db0e` inscrite QUEUE, cible v15.496, 0 changement index.html). **Le diagnostic 08/08 était faux : l'EDL était SOFT-DELETED au cloud depuis le 18/07 09:16 UTC** (Marion ne le voyait pas non plus ; le client masquait correctement). Cause : `saveEDL` reconstruit l'objet → tag `_espaceId` perdu → diff D1 = softDelete chez Marion (octroi gestionnaire = RLS OK) + upsert espace propre skippé. **Portée réelle : toute réédition d'EDL par TOUT utilisateur cloud depuis v15.483** (N=1 tague tout, edl hors REVIVABLE). Fix **D1b réadoption du tag sur la SOURCE** (`store-sync.js`, DRY 1 point, mono inerte, D2 préservé) + **removes suspendus en ambiguïté** (fail-safe homonymes). Preuves : E2E vrai Postgres 7/7 (rouge sans fix) · 16 tests offline · 2197/2197. **EDL `1784120562200` un-deleted (v3)** → visible Marion + Didier dès reconnexion. ⚠️ Jusqu'au déploiement : ne ré-enregistrer AUCUN EDL depuis un compte cloud (consultation/PDF ok). Reste : intégration maître + **E2E réel Didier↔Marion**. Follow-ons : ① résidu préexistant upsert-D2-en-ambiguïté (édition d'une entité tierce homonyme peut écraser l'homonyme propre — SMARTOSAURUS 2 côtés, à instruire) ; ② suite RLS 347/349 = **migration 0044 pas appliquée au DB hébergé** (RESTE connu, feu vert user requis).
