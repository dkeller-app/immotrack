...
| 32 | ARCHI-DB-DOUBLONS | Refonte architecture DB — séparer log (bien physique) et bail (contrat), bidirectionnel | P1 | XL (~12-15h, ~7-10h restantes) | 🔄 **Phase 1 + 2 ✅ livrées v14.14** · Phase 3 ⬜ prête | [docs/subjects/ARCHI-DB-DOUBLONS.md](docs/subjects/ARCHI-DB-DOUBLONS.md) · **Phase 1 CDC** commit `ae73859` (10 décisions arbitrées). **Phase 2 data+getters** commit `511faf3` v14.14 (5 helpers publics getCurrentBailFor/Tenant/Rent + _captureBailSnapshot + _readLogForBail + 12 nouveaux champs sur log dont typeUsage/dpe/etatRisques/mobilier + migration auto au boot avec backup DRIVE-2C). Code legacy intact. **Phase 3 = ~6-8h** refacto reads/writes (splittable 3a UI tabs internes modale logement + wizard bail lecture seule / 3b reads/writes 165 sites PDF + listings). **Phase 4 = ~1-2h** cleanup 33 champs obsolètes. |
