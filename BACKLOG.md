...
> - **⚠️ Divergence index-test à résorber (décidée « après commit PROD » par user)** : les corrections mockup v3 (`b-villeSign`/`b-precedentLoc` en `bp-conditions` + `_bailFillVilleSign`) sont **uniquement en PROD**, PAS dans `index-test.html` (où ces champs restent piégés dans `bp-bien` = régression B7 encore présente). `index-test.html` portait au moment du port la WIP non-commitée d'une session parallèle (« ne pas y toucher »). **À réappliquer dans `index-test` au prochain sync sandbox→PROD** (sinon le sync réintroduit la régression), une fois la session parallèle terminée.
