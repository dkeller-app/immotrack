...
| 5 | REPORTING-BAILLEUR | Onglet **Finances** (analyse) : résultat net + compte de résultat + 4 ratios + argent à récupérer (cliquable→opérationnel) + passerelles 2044/FEC/Bilan · + widget dashboard « Projection »→« Loyers attendu vs encaissé » · [spec](docs/subjects/REPORTING-BAILLEUR.md) | L | ✅ Spec + mockup validés 2026-06-05 → plan d'implé |
...
> - Développé sandbox-first (`index-test.html`, validé live user « ok ça fonctionne ») puis porté **PROD** (`index.html`) sur branche `fix-suppr-immeuble-tombstone` + bump 4 emplacements + `sw.js` CACHE_VER v15.258. **En file d'attente d'intégration index** (`.index-queue/QUEUE.md`) — intégration `index.html`→main par la session maître.
