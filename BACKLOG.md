...
> - **Phase B2** (onglet « 🏷 Diagnostics » unifié dans la modale Logement + retrait écran DDT 360°) **🔄 CODÉ SANDBOX v15.234, EN ATTENTE TEST VISUEL** (2026-06-01) : 1 onglet table groupée fill-once (9 diags + détail DPE + contexte auto-détection) lisant/écrivant le canonique `log.diagnostics` ; moteur `_logDiag*` (~260 l) ; retrait des 2 ex-onglets DPE+Risques de la modale, de l'écran `_renderLogFichePanelDiagnostics` (fiche 360°), de la modale `ov-diag-edit` et des fns `_diagEdit`/`_diagSave`/`_diagSaveContext` (deep-links repointés vers `openLogModalOnTab(ref,'diag')` + redirect compat dans `setLogFicheTab`). **GARDÉ** : moteur de calcul + récap DDT joint au bail (`_ddtRecapOpen`). Projection legacy `log.dpe`(objet)+`log.dpeDate`+`log.etatRisques`(strings) régénérée au commit (`_logDiagCommitToLog`) → bail (`_readLogForBail` + snapshot signé immuable) intact. Audit `superpowers:code-reviewer` : **1 BLOCKER B2-1** (résultats `presence:false`/`conforme:false` sans date silencieusement perdus par le filtre `hasData`) **→ corrigé + 8/8 tests unitaires**, le reste confirmé clean (legal/immutabilité/dead-code/XSS). Syntaxe 0 erreur. **PROD index.html + sw.js PAS encore touchés** (sandbox-first). **Reste B8→B3→B4→B5→B6→B7.**
