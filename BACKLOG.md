...
> - **BAIL-SIGNATURE-DISTANCE** 🔄 (P1, ~10-14h) — signature du bail à distance par le locataire avec **aller-retour 100% automatique** (rejet explicite user de l'approche « envoie un PDF et débrouille-toi »). Architecture : **relais serverless maison (Cloudflare Worker + R2 + KV)** servant `sign.html` (port du wizard Phase 2 existant) + crochet eIDAS qualifié en option premium. Absorbe l'ex-sujet SIGN-BAIL-LIEN (Yousign → premium seulement). **Design V2 consolidé 2026-06-02, attend validation user du spec écrit** avant plan d'implémentation. [docs/subjects/BAIL-SIGNATURE-DISTANCE.md](docs/subjects/BAIL-SIGNATURE-DISTANCE.md) · [mockup V2 interactif — aller-retour auto 3 acteurs](mockups/BAIL-SIGNATURE-DISTANCE/mockup-v2-relais.html) · [mockup v1 obsolète (URL+retour manuel)](mockups/BAIL-SIGNATURE-DISTANCE/mockup.html)
...
> - **Phase B2** (onglet « 🏷 Diagnostics » unifié dans la modale Logement + retrait écran DDT 360°) **🔄 CODÉ SANDBOX, EN ATTENTE TEST VISUEL** :
...
### FEAT-GEORISQUES-ERP 🗺️ — Détection auto ERP/IAL depuis l'adresse (2026-06-02, v15.252-253) — P2 · 🆕 Phase 2 à faire
...
> - **🆕 Phase 2 (À FAIRE, prio à fixer)** — *câblage doc ERRIAL ↔ ligne diagnostic ERP + suivi validité 6 mois*. Aujourd'hui **3 signaux ERP déconnectés** : (a) panneau adresse ✅ détecte · (b) chip scan PDF ✅ « État des risques (ERP) détecté » · (c) ligne DDT « État des risques [ERP] » reste **❓ À renseigner** (couverture scan = indicative par design, n'écrit jamais le résultat). Scope : extraire la date « Établi le » du PDF ERRIAL → suggérer la date ERP « ✨ à vérifier » ; **validité = 6 mois** → date d'expiration ; lier le PDF déposé à la ligne diagnostic ; **croiser la date de validité → message utilisateur « document expiré, régénérez-le gratuitement sur ERRIAL »** (le propriétaire régénère l'ERP lui-même, gratuit, contrairement aux autres diags). Mockup-first + audit code-reviewer obligatoires.
