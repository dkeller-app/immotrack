...
|  | IMPORT-ACTE-VENTE | Création **bailleur + immeuble + logements** depuis l'**acte de vente** (PDF → pdf.js → extraction → wizard validation « suggéré »). Acheteur→entité (rattacher si existe, sinon créer) · adresse + année constr. → immeuble · EDD/lots → logements | P2 | XL | ⬜ Idée — **EN ATTENTE exemples d'actes** | [docs/subjects/IMPORT-ACTE-VENTE.md](docs/subjects/IMPORT-ACTE-VENTE.md) · demande user 2026-06-01 (fil B2 v2-c1) · **verrou : acte = texte libre, pas de format standard (≠ n° DPE)** → arbitrage **A. heuristique locale (gratuit, fragile)** vs **B. LLM (fiable, API/backend, feature SaaS payante)** à trancher après lecture de 2-3 actes réels · réutilise `_ensurePdfjsLoaded`/`_logDiagExtractPdfText` |
