...
- **PIECES-OBLIGATOIRES-BAIL SC1** — checklist des documents obligatoires du bail (2026-07-17, **v15.495**, déployé `f032c38` sur main). Besoin user : à la signature, certaines pièces sont obligatoires à remettre au locataire (DDT loi 89-462 art. 3-3 + règlement de copro) ; il faut savoir lesquelles sont jointes/manquantes et pouvoir les attacher. **Livré** : (1) **moteur pur** `bail-required-docs.js` (11 tests) — par (logement + type de bail) calcule l'état de chaque pièce `ok/miss/verify/na` en réutilisant l'applicabilité tri-état existante `_estDiagApplicable` (CREP <1949, gaz/élec >15 ans, ERP zone… ; champ décisif absent → `verify` « à vérifier », jamais masqué à tort) ; **1 PDF peut couvrir N pièces** (un dossier de diagnostics = DPE+plomb+amiante+gaz+élec) via tag `requirementKeys[]` ; **fichier joint → `ok`** même si applicabilité incertaine (présence prime). (2) **panneau checklist** (variante C validée en mockup) dans l'onglet Bail de la fiche 360 : anneau N/M, groupes À traiter/Jointes, non-applicables repliés avec critère. (3) **modale Joindre multi-couverture** réutilisant le pipeline d'upload existant (`_attachmentSaveForEntity`, IDB+Drive+sync), N/A via le flag `log.diagnostics[key].na` existant (pas de store parallèle). (4) **bandeau signature NON bloquant** (garde `_signatureGuardBanner` enveloppant `openRemoteSignModal`, corps original renommé `_openRemoteSignModalNow` — vérifié adversarialement : aucun chemin ne bloque la signature). (5) **persistance cloud** : colonne `requirement_keys` (jsonb) dans le mapper `documents()` + **migration 0045** (appliquée sur Supabase prod). Stockage au bon niveau : diagnostics→logement (réutilisés par tous les baux successifs), règlement copro→immeuble. **Audit `code-reviewer` PASSANT** (0 Critical, flux signature déclaré sûr ; fix immeuble-sans-id/CSV backfill `nid()`). 2181 tests verts. Boot-smoke navigateur OK (moteur exécuté en live = mockup). Spec `docs/superpowers/specs/2026-07-16-pieces-obligatoires-bail-design.md`, plan `docs/superpowers/plans/2026-07-16-pieces-obligatoires-bail-sc1.md`, mockup `mockups/PIECES-OBLIGATOIRES-BAIL/`. ⚠️ **reste smoke fonctionnel prod = user** (fiche copro → joindre dossier → bandeau signature ; ⚠ cache PWA, hard-refresh). **SUITE = SC2** : envoyer les pièces jointes AVEC le bail signé (présentiel + distance) — approche A validée (PDF unique bail+annexes, pdf-lib déjà chargé). Meublé n'ajoute rien ici (inventaire = EDL).

- **REFONTE-REGUL** — onglet Charges & régularisation (2026-06-23, **v15.350**, `1b92b85`). Retour user : « charges récup non demandées alors qu'il y en a » + « visu par bloc illisible ». Livré : (1) **fix calcul** — `_isChargeRecupCategory` lit le flag `recup` → les charges « Charges récupérables (eau, énergie…) » entrent enfin dans la régul ; (2) **visu** cartes → tableau replié par immeuble + détail par logement (« ce qu'il y a derrière » = vrais mouvements) + **mobile en cartes** ; (3) **détection des trous** (charges non réparties → bandeau) ; (4) **vue globale matrice** par immeuble + **état de validation** (`DB.regulValidations`) + **gate envoi décompte**. Audit `code-reviewer` PASSANT (0 XSS, 0 régression ; 2 points corrigés). Tests charges 21/21. Spec : `docs/superpowers/specs/2026-06-22-regul-charges-refonte-design.md`. ⚠️ validation visuelle sur **vraies données = user** (post-déploiement). Suivis non bloquants : `index-test.html` pré-V3 (sandbox non synchro) ; `_rgValidate` ne trace pas l'auteur ; pas de purge des validations obsolètes. **v2 (2026-06-24, v15.356, `7b1511f`)** : vue globale rendue lisible + responsive suite test prod (libellés bancaires trop longs) — libellé sur 2 lignes, 1re colonne figée + scroll horizontal, clic sur une ligne → `openEditMv` (corriger un mouvement mal catégorisé sans quitter la régul), mobile = liste de charges. ⚠️ vu en prod : des **virements** (« VIR SARAR Tagi », « VIR SCI ALTA remboursement eau ») comptés en charges récup → à re-catégoriser (gonfle le solde) ou décider d'exclure les virements de la régul. **Fix (2026-06-24, v15.359, `d5220a9`)** : éditer un mouvement depuis la régul ne rafraîchissait pas la vue (il fallait quitter/revenir) — `saveMv` appelait `rMv()` (onglet Loyers) ; corrigé via un `case 'regul'` dans `_refreshAfterMutation` qui re-rend la vue globale ouverte (`window._rgGlobalImm`) sinon la liste. Vérifié runtime (solde +20→−380 sans navigation). **Clôture locataire (`378113d`)** : panneau 🏁 par locataire parti — estimation des charges sur N‑1 proratisée à l'occupation (exercice en cours non arrêté) + règle des 20 % DG en copro/collectif (art. 22/23), bouton « Retenir X sur le DG » (`dgRetenu`/`dgRestitue` + `_stamp`). **Assistant de départ Phase 1 (2026-06-30, v15.379, `affac82`)** : fil rouge guidé déclenché depuis le menu ⋮ d'un bail **actif** → stepper 6 étapes (1 congé&préavis · 2 EDL sortie · 3 calcul retenues · 4 restitution DG · 5 régul définitive copro · 6 remise en location), statut DÉRIVÉ des signaux existants (EDL `Sortie`, `dgRetenu>0`/`dgRestitueAt`, `cloture`) ; seul `bail.depart {congePar,congeDate,dateSortie,motif,regulDefFait,remiseLocationFait}` est stocké. Réutilise `_calculerDelaiRestitution`/`_dgStatut`/`_rgImmRegime`/`_dgOpenRestitution`/`openBailClore` (DRY). Bannière compte à rebours du délai légal DG (1 mois conforme / 2 mois retenues ; **max provisoire 2 mois tant que l'EDL n'est pas fait**), garde-fous (étape 4 bloquée sans EDL, étape 6 sans DG restitué). **Audit `code-reviewer` PASSANT** (7 corrections : faux positif étape 3 sur `dgRetenu=0` écrit par `pf()`, débordement `setMonth` fin de mois → clamp dernier jour, libellé délai provisoire avant EDL, garde tombstone bail clos, étape 5 complétable→100 %, libellés exacts art. 22). check-inline-js 5/0, runtime vérifié (dérivation maison/copro, dates limites, round-trip sauvegarde). **Reste** : Phase 2 (CTA deep-link vers clôture/EDL pré-remplis + badge « estimé » dans la régul principale), Phase 3 (remise en location câblée). Mockup approuvé : `mockups/assistant-depart.html`. **VISIBILITÉ (retour user « caché dans le bail, doit être super visible »)** — mockup `mockups/depart-visibilite.html` (3 surfaces réelles) : **#1 carte locataire livré v15.384 (`cb4a066`)** — `bail.depart` présent → carte en état « départ » dans l'onglet Locataires (liseré accent + avancement N/6 + compte à rebours DG, cliquable → `_departOuvrir`) ; sinon bail actif à échéance proche (`ech.urgent`) → bouton « 🏁 Départ » hors du menu ⋮ ; **#2 dashboard « À traiter » livré v15.387 (`2b442be`)** — section dans `_computeUnifiedTodo` : chaque départ actif non terminé remonte comme action priorisée par le délai légal DG (retard → red score 97 « majoration » / J≤15 → ora / sinon info), type `depart` 🏁 dans `_TODO_TYPE_META`, clic → `_departOuvrir`. #1+#2 réutilisent `_departState`/`_departDeadlineDG` (présentation au-dessus du cœur déjà audité Phase 1) → vérif runtime (3 états de carte + items red/info corrects, `_buildTodoDrill`/`_todoV2` OK), check-inline-js 5/0. ⚠️ validation visuelle prod = user. **Reste** : #3 fiche bien 360 (panneau « Départ » — optionnel) ; Phase 2 assistant (CTA deep-link clôture/EDL pré-remplis + badge « estimé » régul) ; Phase 3 (remise en location câblée). **ESTIMATION CHARGES — Phase A livrée v15.393 (`36150bb`)** (retours user : « tu ne vérifies jamais si N‑1 ≈ N ? » + « une facture 1×/an n'est pas ponctuelle » → le vrai signal = récurrence d'une année sur l'autre). Panneau clôture ① refondu : **comparaison N‑1/N‑2/N‑3** (barres + moyenne + flag « année atypique » si N‑1 > moyenne×1,3) ; **liste des dépenses N‑1 par montant décroissant, décochables** (part logement par mouvement via `entry.details`, exclusion persistée `bail.estimExclues`) ; base filtrée × prorata occupation → estimation recalculée live. Occupation = **dérivée des dates** (pas de curseur ; sortie planifiée en réf), déjà via `occDays`/`totalDays`. `_rgN1Charges` réécrit (multi-années + `moves[]`), `_rgYearChargesDetail` nouveau, calcul unifié `_rgClotureCompute` (DRY panneau+doc). **Décompte ESTIMATIF distinct** (`_rgOpenDecompteEstimatif`) du définitif (`openRegulDoc` inchangé) : mentions provisoires + art. 22/23, plus de « exigible sous 1 mois ». Audit `code-reviewer` **PASSANT** (pt1 décompte estimatif corrigé + total=somme parts arrondies + clamp bissextile + commentaires). check-inline-js 5/0, runtime vérifié (multi-années, décochage live, doc estimatif). Mockups : `mockups/cloture-estimation-comparaison.html`. **Phase B livrée v15.400 (`dc30247`)** (retour user : « une alerte avant l'EDL pour demander un acompte de charges »). `_departEstimDu(bail)` : dû estimé du partant HORS écran régul — occupation bornée à la **date de sortie planifiée** (≠ `bail.fin`), réutilise `_rgClotureCompute` sur la fenêtre exercice avec **sauvegarde/restauration de `window._regulFrom/_regulTo`** (0 effet de bord). Alerte acompte **dans le fil rouge** (avant l'EDL, tant que le locataire est présent) : charges estimées / provisions / dû / retenue 20 % / reste hors dépôt + bouton « Demander un acompte » (montant = dû estimé, évite de retenir sur le dépôt) avec **Réviser/Annuler**. `_departDemanderAcompte` trace `bail.depart.acompteDemande` + `mailto` amiable (mention restituable si trop-versé). Item **« À traiter »** escaladé (info→ora) « acompte conseillé ~X € », gaté sur l'EDL (même jalon que l'alerte). Audit `code-reviewer` **PASSANT** (majeur 1 gate EDL harmonisé todo↔alerte + majeur 3 révision/harmonisation `e.du` + mineurs email/console.warn/commentaires ; **majeur 2 = mémoïsation `computeRegul` différée + documentée** pour le scale SaaS — `_departEstimDu` fait 4× computeRegul/partant, OK perso). check-inline-js 5/0, runtime vérifié (dû 295,07 / reste 135,07, restauration fenêtre, demande/révision/annulation, gate EDL). **Phase C livrée v15.403 (`a1eb4d4`)** : acompte RÉELLEMENT encaissé imputé — catégorie dédiée `Acompte de charges (départ)` NEUTRE (ni loyer ni charge récup → zéro double-comptage, 2044/finances nul), acompte dérivé des mouvements (source unique) borné à l'occupation de l'entrée → nette dû/retenue/DG partout (panneau, décompte estimatif, `_departEstimDu`). Bouton « Marquer comme reçu » → `openNewMv` pré-rempli. Audit `code-reviewer` PASSANT (double-compte étanche ; borne occupation + ordre prefill corrigés). 5/0, runtime vérifié (acompte 200 → dû 295→95). Chaîne départ complète bout en bout. **Finition livrée v15.409 (`ce8267c`)** : (1) le **décompte DÉFINITIF** (`_buildDecompteHtml`) + son email + le partage nettent l'**acompte de charges déjà versé** (helper `_regulAcompteDep`, borné à l'occupation) — ligne dédiée + solde corrigé + corps d'email cohérent (ligne conditionnelle `acompteLine`) ; (2) **fiche bien 360** : bandeau « Départ en cours — étape N/6 » (réutilise `_departState`/`_departOuvrir`) ; (3) **assistant deep-link** : étape 3 → `_departOuvrirCloture` (clôture PRÉ-REMPLIE du locataire sur l'exercice de la sortie) + 4 sous-chips de relocation cliquables. La **liste régul reste BRUTE** (cohérence total=Σlignes). Audit `code-reviewer` **PASSANT** (majeur email légal corrigé + borne alignée + liste brute ; effet de bord fenêtre borné, XSS/non-régression OK). check-inline-js 5/0, `email-compose.js` OK, runtime vérifié (email 500+200−600=100). **Reste livré v15.415 (`e2f46dd`)** : (1) **mémoïsation** — cache `_departEstimDu` par (bail+sortie+acompte+exclusions) invalidé par `window._dbGen` (bumpé en 1re ligne de `saveDB`) → les ~8 rendus de `_computeUnifiedTodo` d'un même refresh dashboard = **1 seul calcul par partant** (au lieu de 8×4 computeRegul) ; valeur cachée = objet frais (pas une réf DB), cache d'affichage non-fiscal (la clôture/décompte recalculent toujours à froid). (2) **deep-link EDL** — étape 2 « Faire l'EDL de sortie » → `openNewEDLForLog(ref,'Sortie')` (pré-remplit logement + type, fonction existante). check-inline-js 5/0, runtime vérifié (cache hit ×8 = 4 computeRegul + invalidation post-saveDB + même résultat ; EDL CTA). Non-fiscal → self-vérifié (pas de re-audit agent). **CHAÎNE DÉPART 100 % LIVRÉE.** ⚠️ validation prod = user. **Fix v15.447 (test user RADELET)** : l'alerte acompte s'affiche aussi SANS historique N‑1 — `_departEstimDu` retombe sur les charges RÉELLES (dû réel via `_rgClotureCompute`, path audité) au lieu de disparaître ; libellé « réelles » vs « estimées ». Runtime vérifié (dû réel 178,70). ⚠️ **RESTE À TRANCHER (bug sur-facturation)** : la clôture/décompte d'un partant PAS encore archivé calcule l'occupation sur toute la durée du bail (`bail.fin`), PAS jusqu'à `bail.depart.dateSortie` → sur-facture le partant (ex. RADELET compté sur l'année pleine 2026 alors qu'il part le 01/07). Fix = clipper l'occupation à `dateSortie` dans `computeRegul`/`clipBail` (cœur régul, audit requis) — à valider avec user.
...
> | EM-DASHV4 | **BUG-DASHV4-CLOUD-HYDRATE** — VRAIE cause du bug démo client 2026-07-09 (Accueil cassé : hero météo SVG géant + widgets KO + ancienne sidebar, identique 3 navigateurs, accès libre). `__immoSetDB` remplaçait `DB` en bloc sans rejouer les défauts d'`initDB()` → compte cloud frais/bêta sans `dashRenderV='v2'` → `data-dash-v4="off"` → CSS V4 inerte. Fix DRY `_applyParamDefaults()` rejoué post-hydratation. **Menaçait tout nouvel inscrit bêta.** Audit code-reviewer PASSANT. ⚠️ reste repro user compte frais. | **P0** | S | ✅ Livré v15.452 (`fdc88b5`) |
> | EM-CDN | **FIX-CDN-VENDORED** — durcissement : libs tierces auto-hébergées (Supabase `esm.sh`→`js/vendor/` + `xlsx` cdnjs→`js/vendor/`), plus de CDN externe au runtime. ⚠️ **N'était PAS la cause du bug démo** (voir EM-DASHV4) — durcissement valable (réseau entreprise filtrant + commercialisation SaaS). Suivi : Google Fonts à vendorer. | **P1** | S | ✅ Livré v15.444 (boot `64d2289` + xlsx `14c2539`) |
...
| 5 | REPORTING-BAILLEUR | Onglet **Finances** (analyse) : résultat net + compte de résultat + 4 ratios + argent à récupérer (cliquable→opérationnel) + passerelles 2044/FEC/Bilan · + widget dashboard « Projection »→« Loyers attendu vs encaissé » · [spec](docs/subjects/REPORTING-BAILLEUR.md) · [port PROD](docs/subjects/REPORTING-BAILLEUR-PORT-PROD.md) | L | ✅ **Onglet Finances Livré PROD v15.264** (branche `finance-port-prod`, cherry-pick `7391c26`→`3305fe7`, renum 263→264 car 263 pris par bail-sign-c3). Inclut onglet Finances complet (résultat net, compte de résultat N/N-1, ratios, argent à récupérer, passerelles 2044/Bilan/FEC) + **éditeur correspondance catégories→2044** (`_finCatLigne`/`_finUnmappedCats`). Re-audit `code-reviewer` du diff PROD = **port propre, 0 blocker** · check-inline-js 4/0 · Vitest 1490/47 · render localhost OK. **Widget dashboard « Loyers attendu vs encaissé » ✅ Livré PROD v15.265** (`43a9691`, remplace la carte « Projection 2026 », cherry-pick `2c0dd31`, gates 4/0). **Reste NON porté** : branche `_v4FilterEnt` (prod a `_setActiveEntity` plus récent). ⚠️ `DB.catMapping` interim → converger avec V3-REFONTE-LOYERS Chantier A + **coordination B1** : `_finUnmappedCats` devra exclure `type:'special'` quand les 5 catégories special de B1 arriveront (sinon double-compte). **+ Polish UX Livré PROD v15.293** (`76c04fe`, branche `finance-ux-prod`, cherry-pick `de9e853`, renum 291→293 — course multi-sessions) suite retour user prod : (1) **hero refondu Variante B** (résultat net vedette à gauche + décomposition à droite → fin du « gros bloc »/zone vide ; `.fin-top` grille `1fr 320px` retiré, ratios en **ligne 3 colonnes**) ; (2) **drill-down compte de résultat** `_finDrillLigne` (chaque ligne cliquable → mouvements sous-jacents date/lot·locataire/libellé/montant, totaux **réconciliés** avec `_finChargeBuckets`/`_finLoyersHC`, scope `_finEntScope`, champs user `escHtml`, **read-only**). Audit `code-reviewer` **PASS** (1 bloquant scope `_finEntScope` corrigé) · check-inline-js 5/0. **+ P&L RÉVISÉ — mockup figé 2026-06-19** (`mockups/loyer-2044-pro/pl-revise.html` + vitrine 3 formats `pl-revise-3formats.html`) : bilan de gestion **GLOBAL** tous régimes (nu/meublé/local pro), résultat **AVANT impôts** + caveat indicatif, **enrichissement** (résultat net + capital de crédit amorti = patrimoine), bouton « renseigner les intérêts » (3 méthodes : saisir / modéliser le prêt multi-prêts / importer le tableau d'amortissement), catégories ajoutées CFE/comptabilité/taxe logements vacants, **drill par ligne** enrichi (mouvements/pièces + répartition par bien + graphe N/N-1 + actions, modale plafonnée 88vh), panneau « Argent à récupérer » conservé. ⚠️ **Le régul impacte le résultat** (« pas récupéré, pas de résultat ») = DIVERGE de l'app actuelle (récupérables neutres) → modif `_computeFinancesSummary`/`_finRenderPL`, **audit `code-reviewer` OBLIGATOIRE** au portage. **Plan de portage commité** `docs/superpowers/plans/2026-06-19-pl-finances-refonte-portage.md` (5 phases A→E). **✅ PORTÉ EN PROD 2026-06-20, v15.308→314** (push direct sur `main`, autorisation user, via worktree depuis origin/main) : 308 avant-impôts+caveat · 309 drill enrichi (graphe N/N-1 + répartition par bien) · 310 fix répartition 1-bien · **311 le régul impacte le résultat** (`finances-summary.js` deux étages ; **audit `code-reviewer` : B1 BLOQUANT corrigé** = `_finRegulAFaire` scopé entité, miroir `rRegul` ; 2044/FEC/bilan non impactés) · 312 enrichissement patrimonial (capital amorti = mvts « Prêt — Capital remboursé ») · 313 fenêtre intérêts/crédit `ov-fincredit` (saisir/calculer/importer ; `_amortYear` vérifiée ; ouvre `openNewMv` pré-rempli, pas d'auto-création) · **314 CFE/TLV** = `special`+`gestionCharge:true` (comptent dans le résultat de gestion via bucket `autres`, HORS 2044 par `if(!std.ligne2044)return`) + compta→221 (**audité : isolation 2044 airtight, 0 bloquant, tests `legal-2044`**). **Follow-ups cosmétiques non bloquants** : lignes distinctes CFE/TLV (lumpées dans « autres ») · onglet import tableau d'amortissement (placeholder) · NB-3 écart visible « Résultat de gestion » P&L (inclut CFE/TLV) ↔ « Bilan annuel »/« Voir par logement » (les excluent via `_compute2044`) à documenter/réconcilier. |
...
|---|---|---|---|---|---|
| APL-TIERS-PAYANT | **Prise en compte CAF/APL en tiers payant** (le versement CAF au bailleur compte comme loyer) — à cadrer avec l’utilisateur (rien conçu, diagnostic seulement) | P1 | M | ⬜ À faire | [docs/subjects/APL-TIERS-PAYANT.md](docs/subjects/APL-TIERS-PAYANT.md) |
| CRG-PDF-GERANCE | **PDF compte-rendu de gérance** (agence → propriétaire : paiements reçus, charges, provisions/régul, net reversé) — réutilise moteur unique + chaîne PDF + bloc envoi ; après Phases C-D | P2 | M | ⬜ À faire | [docs/subjects/CRG-PDF-GERANCE.md](docs/subjects/CRG-PDF-GERANCE.md) |
| SUIVI-LOYERS-SOURCE-UNIQUE | **🔥 Un seul moteur de statut de paiement (allocation FIFO + solde) pour toute l'app** — 5 phases A-E (moteur pur TDD → Accueil/Dashboard → Finances/nav → Quittances/Pilotage → Régul) ; corrige constats 4-9 + 40-45 de l'audit (6 formules → 1, faux impayés, régul gonflée sur paiement groupé) ; mockup validé user 2026-07-08 | P1 | M-L | 🔄 A ✅v15.425 · B ✅v15.427 (Accueil+Dashboard) · D-matrice ✅v15.451 (Pilotage : cumul borné anti-fantôme, avance visible, DG « non suivi » vs alerte) — audits PASSANT · **reste C (Finances+nav), D-quittances, E (régul)** | [docs/subjects/SUIVI-LOYERS-SOURCE-UNIQUE.md](docs/subjects/SUIVI-LOYERS-SOURCE-UNIQUE.md) |
| AUDIT-FINANCES-COHERENCE | **🔥 Audit complet page Finances : 4 moteurs de calcul incohérents entre eux** (déclencheur user : Solde charges récup −1 957,03 vs widget Régul +2 137,00 sans explication) — ~39 constats (3 critiques, ~15 importants), rapport complet + plan de correction en 3 lots | P1 | M-L | ⬜ Audit livré 2026-07-07, corrections à arbitrer | [docs/subjects/AUDIT-FINANCES-COHERENCE-2026-07-07.md](docs/subjects/AUDIT-FINANCES-COHERENCE-2026-07-07.md) · 4 audits agents parallèles sur v15.423 · aucun bug d'arithmétique, mais total « Argent à récupérer » = somme de valeurs absolues d'unités hétérogènes (les signes affichés donnent 10 042,13, pas 18 891,67), impayé : montant et compte « 14 locataires » de 2 formules incompatibles, IRL 0,00 trompeur (table IRL pas à jour → tout à 0), ligne « Autres charges » comptée mais invisible du tableau v2, intérêts 31/12 invisibles de l'exercice, mapping user ignoré par la 2044 |
...
| V3-REFONTE-LOYERS | Refonte fonctionnelle onglet Loyers/Mouvements | P2 | M-L | 🔄 Chantier A ✅ + B1 ✅ / B2-B3-C à venir | **Chantier A (2044 moteur unifié & juste) TERMINÉ PROD** : Phase 1 v15.259 + Phase 2 v15.261 (wizard 2044 ne plante plus, un seul moteur, chiffres justes). **B1 (catégories) ✅ PROD v15.267** (`5c80b9a` : +5 catégories `type:'special'` hors résultat foncier — construction/agrandissement non déd., acquisition/cession, CCA/distribution SCI, virement interne, régul de solde locataire — + précédence `special` sur catMapping = anti double-compte). Reste **Chantier B2-B3** (affectation pilotée par catégorie : UI sur vrai STD_CATEGORIES + cœur UX 1 question par catégorie) + **Chantier C** (import→liste→split multi-sens). plan `docs/superpowers/plans/2026-06-08-chantier-a-2044-unifie.md` · spec `docs/subjects/V3-REFONTE-LOYERS.md` · **Maquettes refonte fiscale déployées (2026-06, `mockups/loyer-2044-pro/`)** : import bancaire (affectation), **2044 = lentille de l'onglet Finances** (prévisu→valider→générer, saisie ligne 250), documents par entité, crédit/amortissement — direction design validée. Renommage onglet Loyers→Mouvements reste à faire (cf 19D). Lié au **P&L révisé** (cf REPORTING-BAILLEUR). |
...
- **CI-VITEST-MAIN-ROUGE (investigué 2026-08-10, déjà résolu)** : les 356 notifications d'échec (constat user 2026-08-08) sont un historique — **main est déjà VERT depuis le 2026-07-13**. Cause racine : la restructuration des catégories 31→21 (`94445a5`, 2026-06-21, v15.326) a renommé/fusionné des libellés (ex. « Frais de comptabilité / expert-comptable » → « Frais de gestion / honoraires / comptabilité », « Régularisation de solde » → « Divers (non déductible) », CFE/TLV sorties du standard) sans mettre à jour les assertions figées dans `__tests__/helpers/bank-import.test.js` et `legal-2044.test.js` → CI rouge sur **chaque push** du 2026-06-25 (run #805) au 2026-07-12 (run #967, 163 runs rouges consécutifs). **Fix déjà livré le 2026-07-13** par une session antérieure (commit `be56dac`, branche `claude/fix-ci-tests-2044`) : réalignement des assertions sur les nouveaux libellés avec matching par alias (résout aussi la parité `index.html`/`index-test.html`, ce dernier figé pré-v15.314), + nouveau test anti-régression sur le fail-safe CFE/TLV (catégorie custom non mappée → exclue + remontée `nonMappes`). Sémantique fiscale vérifiée avant/après, 1874/1874 verts en local à l'époque. **Vérifié 2026-08-10** : 39 runs verts consécutifs (#968→#1006), y compris le tout dernier push du jour (`43f4705`, v15.495) — confirmé via l'API GitHub Actions (`gh` CLI absent du poste, requêtes faites en HTTP direct, repo public). Le scope CI (`vitest.config.js` → `__tests__/**/*.test.{js,mjs}`) exclut déjà proprement `supabase/tests/*` (suite RLS Postgres réel, lancée séparément via `npm run test:rls`) — aucun changement nécessaire là-dessus, c'était déjà bien scopé. **Reproduit en local** (worktree `origin/main`) : `npm install` + `npm run test:run` → 2181/2181 verts + `check-inline-js.mjs` OK, conforme au job CI. Aucun test affaibli, aucun fix nécessaire côté workflow — seul ajout : cron quotidien keep-alive Supabase (voir `docs/subjects/P0-SUPABASE-PAUSE.md` option B) dans `.github/workflows/supabase-keepalive.yml`.
