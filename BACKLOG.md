...
>     - **B2 v2-b** (à faire, sensible) : ADEME auto-fetch (n° DPE → API `data.ademe.fr` → classe) + 2 pop-ups de garde (adresse ≠, données ≠) → audit agent.
>     - **B2 v2-c** (à faire, fragile) : lecture PDF native via `pdfjsLib` (regex n° + dates + conclusions) → pré-remplissage « suggéré » → audit agent.
>   - **PROD index.html + sw.js PAS encore touchés** (sandbox-first), sync différée jusqu'à validation B2 v2 complète. **Reste B2v2-b/c puis B8→B3→B4→B5→B6→B7.**
