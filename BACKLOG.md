...
| 32 | ARCHI-DB-DOUBLONS | Refonte architecture DB — séparer log (bien physique) et bail (contrat), bidirectionnel | P1 | XL (~12-15h, ~3-5h restantes) | 🔄 **Phases 1+2+3a+3b ✅ livrées v14.16** · Phase 4 ⬜ cleanup | [docs/subjects/ARCHI-DB-DOUBLONS.md](docs/subjects/ARCHI-DB-DOUBLONS.md) · **Phase 1 CDC** `ae73859`. **Phase 2 data+getters** `511faf3` v14.14. **Phase 3a UI tabs** `5d7097f` v14.15. **Phase 3b sync étendu** `17426cf` v14.16 (PIVOT stratégique : au lieu de refacto manuelle 149 sites, on étend `_syncLogToBail` pour propager 24 nouveaux champs bien depuis log vers bail courant, skip si signé pour immutabilité, code legacy intact). **Phase 4 = ~3-5h** vraie refacto reads/writes (149 sites → `_readLogForBail` + getCurrentBailFor/Tenant/Rent dans rBaux, rMv, rDash, rEDLList, rQuit, fiches 360°) + 5 fonctions PDF + suppression définitive 33 champs `bail.X` obsolètes + suppression `_syncLogToBail`. |
