...
> | P1 reste : conflit→re-hydrate + récepteur Realtime `changed` + re-pull `visibilitychange` + purge cache logout/révocation (fuite RGPD miroir) + défauts espace frais (onboarding cassé pour TOUT nouvel inscrit). ⚠ Reprendre au chantier conflit→re-hydrate les réserves d'audit **M2** (remove en conflit permanent ⇒ `_hasPendingRemoves` vrai à chaque save ⇒ debounce neutralisé tant que le conflit vit → exclure les clés dont la dernière tentative fut `conflict`) et **M4** (broadcast Realtime jamais émis tant qu'un poison traîne → émettre si `upserts.length` ou `removes.length` quand un récepteur sera rebranché) | P1 | L-XL | ⬜ sessions dédiées |
