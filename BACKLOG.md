...
| 5 | REPORTING-BAILLEUR | Onglet **Finances** (analyse) : résultat net + compte de résultat + 4 ratios + argent à récupérer (cliquable→opérationnel) + passerelles 2044/FEC/Bilan · + widget dashboard « Projection »→« Loyers attendu vs encaissé » · [spec](docs/subjects/REPORTING-BAILLEUR.md) | L | 🔄 Sandbox livré + audité 2026-06-05 (commits 9db909c→4e041d6) · attend validation user avant PROD |
...
| V3-REFONTE-LOYERS | Refonte fonctionnelle onglet Loyers/Mouvements | P2 | M-L | 🔄 Chantier A ✅ / B-C à venir | **Chantier A (2044 moteur unifié & juste) TERMINÉ PROD** : Phase 1 v15.259 (49ba4ff : fix ligne 230 deduction + builder opts complet) + Phase 2 v15.261 (54fd084 : suppression inline `_compute2044` + adaptateur wizard sur moteur module + libellé 225). Wizard 2044 ne plante plus, un seul moteur, chiffres justes. Reste **Chantier B** (affectation pilotée par catégorie) + **Chantier C** (import→liste→split multi-sens). plan `docs/superpowers/plans/2026-06-08-chantier-a-2044-unifie.md` · spec `docs/subjects/V3-REFONTE-LOYERS.md` |
...
### BUG-RECREATE-REF-TOMBSTONE — bien recréé avec une réf supprimée restait invisible (2026-06-08, v15.260)
...
> - Sandbox-first (`index-test.html`, OK user) → port **PROD** sur branche `fix-recreate-ref-tombstone` (base `origin/main`) + bump **v15.260** (4 spots + `sw.js`). **⚠️ v15.260 et non 259** : `origin/main` portait un demi-bump (Chantier A Phase 1 `49ba4ff` avait mis title/`<em>` à 15.259 mais laissé footer/const à 15.258) → pris 15.260 ET réparé l'incohérence (4 spots cohérents). **En file d'attente d'intégration index** (`.index-queue/QUEUE.md`).
