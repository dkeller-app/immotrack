...
| ARCHI-MODULAR | Refonte modulaire monolithe index.html (ES modules natifs) | P1 | XL | 🔄 Phase 0 livrée v14.85 (Sprint 2 marathon) | [docs/strategie/ARCHI-MODULAR-FAISABILITE.md](docs/strategie/ARCHI-MODULAR-FAISABILITE.md) · **Stratégie 2 ES modules natifs**. **Phase 0 v14.85** : CSS extrait dans `css/main.css` (3434 lignes), skeleton `js/main.js` créé avec marqueur `window.__IMMOTRACK_MODULE_BOOTSTRAP__`, `<link>` + `<script type="module">` injectés dans index-test.html. -3434 lignes dans le monolithe. Phases 1-4 (core/components/3 tabs simples/cleanup) en cours dans la même session marathon. **Pré-requis dev** : `npx http-server` sur port 8766 (modules ES bloqués en file://). |
