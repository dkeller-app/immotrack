...
> | **RESET-CLOUD UX** — « réinitialiser » local ne vide PAS l'espace cloud (design : pas d'auto-suppression, preuves légales) → reconnexion re-télécharge tout ; user confirme 13/07 : « bouton qui ne sert à rien ». À livrer : en mode cloud, wording honnête « Vider cet appareil (les données cloud restent) » + vrai « ⚠️ Vider mon espace cloud » (double confirmation + backup auto préalable, via `purge_espace` en Edge Function ou RPC gardée). **Session quick-wins cloud UX** avec BANDEAU-SAUVEGARDE-CLOUD | **P1 ⬆ (user)** | ⬜ |
