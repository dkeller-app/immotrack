...
|  | IMPORT-ACTE-VENTE | Création **bailleur + immeuble + logements** depuis l'**acte de vente** (PDF → pdf.js → extraction → wizard validation « suggéré »). Acheteur→entité (rattacher si existe, sinon créer) · adresse → immeuble · constitution → logements | P2 | L | 🔎 **Analysé (4 actes réels) — verdict voie A** | [docs/subjects/IMPORT-ACTE-VENTE.md](docs/subjects/IMPORT-ACTE-VENTE.md) · demande user 2026-06-01 (fil B2 v2-c1) · **analyse empirique 2026-06-01 : actes notariés = modèle NORMALISÉ → forte régularité** → **voie A (heuristique locale, gratuite, sans backend) RETENUE** : entité (nom+**SIREN** `numéro unique d'identification`)+immeuble (adresse) fiables, **logements en brouillon « suggéré » éditable** (lots≠logements : réunions/caves/parkings → validation user), **année constr. ABSENTE des actes** (→ DPE). B (LLM) = upgrade SaaS futur. Effort XL→L. Prêt à brainstormer wizard · réutilise `_ensurePdfjsLoaded`/`_logDiagExtractPdfText` |
