...
- 2026-05-20 : test user → FAB figé sur « Sauvegarde… » + cross-tab non propagé. Console : aucune erreur/retry → **hang silencieux** d'une requête Drive (Opera + VPN), aucun timeout sur fetch → `_driveSaving` bloqué à true pour toujours. ✅ Fix v15.117 : timeout 30s `AbortController` sur `driveFetch` → au hang, abort → retry/erreur → FAB se débloque, data safe en local. Confirme piste A5 (VPN/Opera bloque googleapis). Cross-tab instant (au-delà des signatures) = enhancement séparé à décider.
