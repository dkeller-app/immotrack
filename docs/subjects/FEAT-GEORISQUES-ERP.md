...
> **Extraction date** : date d'établissement extraite **séparément** (`erpDate`/`erpDateRaw`/`erpDateSrc` dans `_logDiagExtractSuggestions`) et appliquée **à la clé `erp` uniquement** dans `_logDiagApplySuggestions` (`key==='erp' && sug.erpDate ? sug.erpDate : sug.date`) → suggérée « ✨ à vérifier » (jamais d'écriture aveugle), zéro pollution DPE/plomb/gaz/autres. DPE jamais suggéré.
>   - **🐛 Correctif format (test visuel 2026-06-02, sur ERRIAL réel)** : le modèle officiel ERRIAL (georisques.gouv.fr) écrit la date en **TEXTE français** — « **Établi le 2 juin 2026** » — et NON en numérique `JJ/MM/AAAA` comme supposé initialement (d'où détection couverture ✅ mais date non remplie). L'extraction gère désormais (a) le numérique `établi le 02/06/2026` (ERP de diagnostiqueurs) **et** (b) le textuel `établi le {1er|2..31} {mois en lettres} {AAAA}` (mois accentués/non, « 1er » toléré), validation calendaire `Date.UTC` partagée. Distracteur « établi **pour** les parcelles » correctement ignoré. Prouvé sur 7 cas + re-audité `superpowers:code-reviewer` ✅ (0 Critical/0 Important).
>   - **Follow-up (non bloquant, P3)** : extraire le parseur de date ERP en helper testé Vitest (la regex légale n'a aujourd'hui de couverture que manuelle, fonction inline dans le monolithe).
