...
- **2026-06-08 (suite 7 — Chantier A : planification + découverte « 2 moteurs 2044 vivants », pas 1 mort)** : en lisant le code réel avant d'écrire le plan, le « wizard inline mort » de l'audit s'est révélé **vivant et cassé**, pas mort. État réel :
  ...
  - **Le module est déjà juste** (gère `deduction` l.94, `imm` l.51, forfait 222 l.108, partBailleur 225 l.116, skip compteur l.70). Les bugs sont **côté câblage prod** : ligne 230 `type:'charge'` (l.3969) + `_legal2044BuildOpts` **incomplet** (l.43193, ne passe pas `imms/nbLocaux/partBailleur225`). **Le sandbox a déjà les deux correctifs** (l.3904 deduction + l.42851 builder complet) → Phase 1 = simple port test→prod.
  - **L'inline est un superset** (détail des mvts par ligne pour le PDF, mapping custom, déficit) → « unifier » = migrer ces 2 capacités **dans le module** (opt-in `detail` + `mapping`), écrire un **adaptateur UI** qui reshape la sortie module vers la forme `byLine` du rendu/`_print2044`, puis **supprimer l'inline**. `LIGNES_2044` reste (libellés UI, pas un moteur).
  - **Décision user (AskUserQuestion)** : sur les 3 options (brancher l'ancien sur le module / retirer l'ancien / juste les chiffres), le user a choisi **« Brancher l'ancien sur le bon moteur »** — UI wizard + PDF conservés, branchés sur le module, zéro feature perdue.
  - **Avancement 2026-06-08** : **Phase 1 ✅ PROD v15.259** (commit 49ba4ff : ligne 230 deduction + builder opts complet + test garde-fou). **Phase 2 moteur ✅** (module `js/core/legal-2044.js` : `opts.detail` commit 606ad7e + `opts.mapping` commit 1c860c5, TDD, 38 tests verts). **Phase 2 sandbox ✅** dans `index-test-loyer.html` (sandbox dédié, commit 525022e) : adaptateur `_legal2044WizardData` + rewire wizard/`_print2044` + suppression inline `_compute2044` (commit 9491dd6). **Audit agent fiscal = PASS** (0 régression chiffres) ; 2 correctifs d'affichage appliqués (libellé part bailleur 226→225, mvtsCount inclut non-mappés ; commit e3a4de8). **Smoke test navigateur OK** : wizard ouvre + récap rendu sans erreur, parité moteur confirmée, 0 erreur console. **Phase 2 PROD ✅ livrée v15.261** (commit `54fd084` sur `origin/main`) : inline `_compute2044` supprimé + adaptateur wizard sur moteur module + libellé 225 + bump 4 spots/`sw.js` (répare aussi le demi-bump Phase 1). Smoke test `index.html?sandbox=1` : parité wizard↔panneau OK, 0 erreur. **⚠️ Intégré DIRECT sur main par accident** (course multi-sessions sur l'arbre partagé : une autre session a fait `checkout main` entre mes edits et le commit → tombé sur main au lieu de la branche ; historique linéaire, rien détruit ; détail + conséquences coordination dans `.index-queue/QUEUE.md` journal 2026-06-08, dont **renumérotation du P1 fix-recreate-ref-tombstone v15.260→v15.262**). **Chantier A = TERMINÉ.** Reste validation user sur vraies données (le bouton 📋 Wizard 2044 ne plante plus + chiffres justes).
  - **Plan détaillé** : `docs/superpowers/plans/2026-06-08-chantier-a-2044-unifie.md` — Phase 1 (panneau juste en prod, port sandbox→prod, gate OK user) + Phase 2 (moteur unifié sandbox-first : module `detail`+`mapping` en TDD → adaptateur → rewire wizard → suppression inline → **audit agent fiscal obligatoire** → port prod après OK). Versions : Phase 1 v15.259, Phase 2 v15.260.
