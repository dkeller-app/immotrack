...
- 2026-05-25 : **POC v15.167 validé empiriquement** (Marion accède aux binaires PJ via permissions.create). **v15.168 Phase 1-5 industrialisation** : UI Co-gestionnaires + auto-share + plan « 1 fichier par user » + 21 tests Vitest. **v15.169 fix folders=writer** (Marion peut uploader). **v15.170 fix `_drvSAD()` manquant** sur 5 endpoints binaires (vrai bug racine du « Binaire introuvable »). **v15.171 DRIVE-REORG Phase A** : docs entité/immeuble cloisonnés par entité. **v15.172 DRIVE-REORG Phases B+C+D+E** : sauvegarde JSON `[entité]/`, auto-détection cross-user zero-friction, migration bouton, UI refondue. **8 commits Drive sur cette session — sujet bouclé, à tester en prod**.
