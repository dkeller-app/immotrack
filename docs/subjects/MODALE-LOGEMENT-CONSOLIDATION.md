...
- **2026-06-01 — Phase B3 CODÉE SANDBOX (v15.246, `index-test.html` uniquement)** — Bâti → immeuble hérité + fallback (plan item **D-B1**). Le `type d'habitat` rejoint la période de construction et le régime juridique comme propriété portée par l'IMMEUBLE, héritée (lecture seule) dans la modale Logement, avec fallback éditable si le logement est autonome (pas d'immeuble lié).
  - **Immeuble** : nouveau `<select id="imm-typeHabitat">` (Immeuble collectif / Maison individuelle) sur la même ligne `.fg2` que le régime juridique (nbLots passe en bloc autonome). Câblage complet : migration boot (`if(!('typeHabitat' in im)) im.typeHabitat=''`), `resetImmeuble`, `editImm` (load), `saveImmeuble` (persist avec fallback `existingIm?.typeHabitat`).
  - **Logement (onglet Description)** : les 3 champs bâti (`log-typeHabitat`/`log-regimeJuridique`/`log-periodeConstr`, persistance editLog/reset/saveLog **inchangée**) sont enveloppés dans `#logmod-bati-own` (`display:none` par défaut). `_logRefreshInherited()` bascule via le helper `_toggleBati(ownVisible)` : **autonome** (pas d'entité/immeuble) ou **immeuble parent introuvable/supprimé** → bloc éditable visible (saisie sur le logement) ; **immeuble lié** → bloc masqué + note « 🏛 hérité » + valeurs affichées en lecture seule dans l'encart « Hérité de l'immeuble parent » (onglet Identité). Nouvelle ligne « Type d'habitat » dans l'encart (HTML statique + version reconstruite par JS), alimentée par `setT('logmod-inh-typeHabitat', imm.typeHabitat || '—')`.
  - **Bail** : `_readLogForBail` résout `_inhTypeHabitat = _imm.typeHabitat` dans les 3 branches (resolver / inline-autoritative / partielle — lecture inline directe, **PAS via le resolver partagé** `log-immeuble-resolver.global.js` pour préserver l'isolation sandbox, ce fichier étant aussi chargé par la PROD), expose l'alias `_immTypeHabitat` ET **override `typeHabitat: _inhTypeHabitat || src.typeHabitat || ''`** dans le littéral de retour → les 6 sites de lecture bail (`b-typeHabitat` prefill, `isCopro`, PDF/preview) reçoivent automatiquement la valeur héritée. **Immutabilité bail signé préservée** : `_imm` vient de `bailSnapshot.imm` figé (aucun lookup DB mutable).
  - **Vérif** : parse JS des 4 blocs **valide (0 erreur)** · **1319/1319 Vitest** · grep de cohérence des 11 sites câblés (form imm, boot, reset/edit/save imm, toggle 3 branches, setT, encart ×2, `_readLogForBail` ×4). **B3 NON listée phase sensible** (pas d'audit code-reviewer obligatoire) → EN ATTENTE TEST VISUEL user sur `index-test.html`. Reste : B4→B5→B6→B7.
