...
- 2026-06-05 : **dataset démo + 2 corrections de chiffres** (découvertes en préparant la démo). (1) **loyers HC** : l'app enregistre le loyer charges comprises (ligne 211 = bruts) → le résultat net comptait les provisions du locataire comme revenu sans déduire la charge récupérable en face (surévaluation). Corrigé via `_finLoyersHC` qui répartit chaque loyer encaissé entre HC et provisions selon le ratio hc/(hc+ch) du bail (choix user : split exact par logement, vs proxy 211−229). (2) **résultat net N-1** recalculé sur la même base que N (buckets, récupérables exclues, intérêts inclus) → variation cohérente. (3) **occupation** : nbOcc fractionnel → taux pondéré dans le temps (87 %) au lieu d'un décompte arrondi (75 %). Dataset démo refondu (2024+2025+2026 YTD, catégories STD mappées 2044, scénarios vacance/impayé). Vérifié bout-en-bout au navigateur (net 13 860 €, recouvrement 92,6 %, occupation 87 %, vacance 4 231 €, impayé 1 940 €). Re-audit code-reviewer : OK. Commit `1e49254`. Démo chargeable via bouton « 🎲 Charger dataset démo » du bandeau sandbox.
...
- 2026-06-08 : **sandbox dédié `index-test-finance.html`** créé (isole le travail Finances des éditions concurrentes du `index-test.html` partagé ; même pattern que `index-test-loyer.html`). Corrige au passage : page noire (2ᵉ détection test-mode `data-lpboot` oubliée), menu Finances absent (ajouté au menu ACTIF `v4s-sb-nav`, pas seulement au `.ni` caché). **3 retours user livrés** : (1) **sélecteur d'année** (4 exercices, défaut clos, avertissement « en cours »), (2) **filtre par entité** (rFinances scope sur la pastille active — résultat net/ratios/compte de résultat/impayés/IRL ; helpers `_finEntScope`/`_finInScope`), (3) **clic pastille reste sur Finances** (`_v4FilterEnt` branche `currentPage==='finances'`). Démo enrichie à **2 SCI** (filtre démontrable : Toutes 18 860 / SCI DEMO 13 860 / SCI BIS 5 000). Vérifié navigateur localhost.
- 2026-06-08 : **éditeur de correspondance catégories→2044** (interim) — découvert en testant sur **données réelles user** : 173 mvts tous en 2026, catégories toutes **≠STD** (`Loyers`,`Prêt`,`Gestion`,`Travaux/Réno`,`Assurances PNO`,`Charges`…) → compte de résultat à 0 (le moteur 2044 mappe par nom de catégorie standard, et `DB.categories` n'a pas de `ligne2044`). Trou fiscal **transverse** (touche aussi 2044/Bilan), capturé → `CAT-MAPPING-2044.md` **fusionné dans V3-REFONTE-LOYERS Chantier A** (la session loyers a l'éditeur unifié prévu ; mais occupée → fait ici en interim). Implé : `DB.catMapping {cat:ligne}` + résolveur `_finCatLigne` (STD+mapping), `_finLoyersHC`/`_finChargeBuckets` consultent le résolveur, bannière « N catégories non reliées → Associer », modal éditeur avec **suggestion auto validée par l'user** (jamais d'auto-mapping silencieux). **Audit `code-reviewer`** : 1 blocker corrigé (escaping clé `data-cat` via `escHtml`), + scope imm aligné sur `_compute2044`, + net db−cr. Vérifié : avant 0 € → après mapping +2 060 € (récupérables 229 exclus, HC splitté). Persiste au reload. **User : « ok ça fonctionne »**. Commits locaux `b42d34c`→`0922ffd` (⚠ **push origin BLOQUÉ** par la concurrence des sessions sur l'arbre partagé — testé sur localhost en attendant ; à pousser quand le dépôt est calme). **Reste gaté** : wiring PROD `index.html` (D5) + adoption du schéma `catMapping` par la session loyers.
