...
- 2026-05-03 (nuit) : fix mobile v14.37 (volet 5) · pulse SVG → DIV flexbox (cause : iOS Safari mobile rend mal les SVG très plats avec `preserveAspectRatio="none"` + height fixe `11px`) · cellules `.lhp-cell` en `flex:1`, hauteur 8px desktop / 10px mobile (touchable) · marker mois courant en `box-shadow` 3px var(--acc) · responsive 600px préserve la grille du hero compact 64px/1fr
