...
- 2026-05-21 : **v15.142 — refonte en bottom-sheet drawer** (décision user après échec de la sidebar overlay sur iPhone, même en v15.141). Abandon de l'overlay sidebar latérale. Nouvel élément dédié `#more-ov` : drawer qui glisse depuis le bas (z-index 2100), grille des 13 fonctions hors bottom nav (emoji + label), backdrop tap = ferme, ✕ = ferme, tap fonction = navigue + ferme. Le bouton "Plus" ET le burger du haut (`toggleSb` sur ≤768px) ouvrent ce drawer. Vérifié dans preview Chromium 375px : ouverture, navigation+fermeture, clic backdrop ferme, clic feuille ne ferme pas, burger haut ouvre aussi. ⚠️ Test iPhone user à confirmer (cette fois sans dépendance position:fixed sidebar). Commit 2a2e952.
